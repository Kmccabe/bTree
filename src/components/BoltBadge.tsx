...
const BoltBadge: React.FC<BoltBadgeProps> = ({ variant = 'default', className = '' }) => {
  ...
  const variants = {
    ...
    hero: "px-6 py-3 bg-gradient-to-r from-purple-600 via-blue-600 to-indigo-600 text-white rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-1"
  };

  const getBoltIcon = () => {
    switch (variant) {
      case 'hero':
        return <Zap className="w-5 h-5 text-yellow-300" fill="currentColor" />;
      case 'default':
        return (
